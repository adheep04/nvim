...
-- Recursive unfold function
local function recursive_unfold()
  local line = vim.fn.line('.')
  local fold_end = vim.fn.foldclosedend(line)
  if fold_end == -1 then return end  -- Not on a closed fold

  local initial_level = vim.fn.foldlevel(line)
  vim.cmd('normal! zO')  -- Open fold under cursor

  -- Recursively open all folds within this fold 
  local curr_line = line
  while curr_line <= fold_end do
    if vim.fn.foldlevel(curr_line) > initial_level then
      if vim.fn.foldclosed(curr_line) ~= -1 then
        vim.cmd(curr_line .. 'foldopen')
      end
    end
...
-- Colemak navigation
map({ "n", "v", "o" }, "j", "h") -- map j to h (left)
map({ "n", "v", "o" }, "k", "j") -- map k to j (down)
map({ "n", "v", "o" }, "l", "k") -- map l to k (up)
map({ "n", "v", "o" }, ";", "l") -- map ; to l (right)
map({ "n", "v", "o" }, "'", ":", { silent = false })  -- map ' to :
map({ "n", "v", "o" }, "f", "/", { silent = false })
n('<leader>f', 'za', { desc = 'toggle fold' }) -- map F to toggle fold

-- Line manipulation
n('<leader>L', 'yyP', { desc = 'duplicate line up' })
n('K', ':m .+1<CR>', { desc = 'move line down' })
n('L', ':m .-2<CR>', { desc = 'move line up' })
n('<leader>K', 'yyp', { desc = 'duplicate line down' })

-- Swap s and v functionality
map({ "n" }, "s", "v") -- map s to visual mode
map({ "n" }, "v", "s") -- map v to substitute character
map({ "n" }, "S", "V") -- map S to visual line mode
map({ "n" }, "V", "S") -- map V to substitute line

-- Swap 'a' and 'i' functionality
map({ "n" }, "a", "i") -- map 'a' to insert mode (before cursor)
map({ "n" }, "i", "a") -- map 'i' to append mode (after cursor)
map({ "n" }, "A", "I") -- map 'A' to insert at beginning of line
map({ "n" }, "I", "A") -- map 'I' to append at end of line
...
map('v', '<M-k>', ":m '>+1<CR>gv=gv", { desc = 'move selection down' })
map('v', '<M-l>', ":m '<-2<CR>gv=gv", { desc = 'move selection up' })
...
-- recursive_unfold
n('<leader>F', recursive_unfold, { desc = 'recursively unfold from cursor' })
...
local plugins = {
  ...
  -- Diagnostics and errors display
  {
    "folke/trouble.nvim",
    ...
    config = function()
      ...
      n("<leader>xx", function() require("trouble").toggle() end, { desc = "toggle diagnostics list" })
    end
  },
  ...
  -- Treesitter syntax highlighting
  {
    ...
    config = function()
      require("nvim-treesitter.configs").setup({
        ensure_installed = { "lua", "python", "vim", "query" },  -- Reduced list
        highlight = {
          enable = true,
          disable = function(lang, buf)
            local max_filesize = 200 * 1024 -- Increased to 200KB
        ...
        indent = { enable = true },
        incremental_selection = { enable = false },  -- Disable unused modules
...

  {
    ...
    keys = { "zR", "zM" },
    config = function()
      vim.o.foldcolumn = '3'  -- Increased from 1
      vim.o.foldlevel = 99
      ...
      n('zR', require('ufo').openAllFolds, { desc = 'ufo: open all folds' })
      n('zM', require('ufo').closeAllFolds, { desc = 'ufo: close all folds' })
    end,
  }, 
 -- LSP configuration (for Python and other languages) - Streamlined
  ...
  -- Simplified LSP setup
  {
    ...
    config = function()
      ...
      n('<leader>dk', vim.diagnostic.goto_next, { desc = 'next diagnostic' })
      n('<leader>dl', vim.diagnostic.goto_prev, { desc = 'previous diagnostic' })
      n('<leader>D', function()
...
-- Simplified Python environment manager
do
  local M = {}
  local envs_dir = vim.fn.expand("~/envs")

  function M.discover_environments()
    local environments = {}
    local paths = vim.fn.glob(envs_dir .. "/*", false, true)
    for _, path in ipairs(paths) do
      local python_bin = path .. "/bin/python"
      if vim.fn.isdirectory(path) == 1 and vim.fn.executable(python_bin) == 1 then
  ...
  function M.set_python_env(env_path)
    ...
    vim.defer_fn(function()
      ...
      vim.notify("Activated: " .. vim.fn.fnamemodify(env_path, ":t"))
    end, 100)
  end

  function M.select_env()
    local envs = M.discover_environments()
    if #envs == 0 then
      vim.notify("No Python environments found in " .. envs_dir)
      return
    end
    ...
    vim.ui.select(items, {
      prompt = "Select Python environment:",
        ...
        for _, env in ipairs(envs) do
          if env.name == selected then
            M.set_python_env(env.path)
            break
          end
        end
      end)
  ...
  n("<leader>vs", M.select_env, {desc = "Select Python environment"})
  vim.api.nvim_create_user_command("PythonEnvSelect", M.select_env, {})
...
vim.api.nvim_create_autocmd("FileType", {
  ...
  callback = function()
    ...
    -- Try to detect python path from environment
    if vim.env.VIRTUAL_ENV then
...
vim.api.nvim_create_autocmd("BufWritePost", {
  group = vim.api.nvim_create_augroup("NoFoldOnSave", { clear = true }),
  callback = function()
    vim.defer_fn(function() vim.cmd("normal! zR") end, 50)
  end,
})



-- Add this to solve the "No fold found" issue
